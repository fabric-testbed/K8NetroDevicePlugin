...
func (pci PCILister) Discover(pluginListCh chan dpm.PluginNameList) {
	var devicesSet = make(map[string]DeviceStruct)
	var vid string
	var did string
	filepath.Walk("/sys/bus/pci/devices", func(path string, info os.FileInfo, err error) error {
		...
		vid, did, err = getDeviceVendor(info.Name())
		if err != nil {
			glog.V(3).Infof("Could not process device %s", info.Name())
			return filepath.SkipDir
		}
		if vid != "19ee" {
			glog.V(3).Infof("Ignoring cards other than Netronome%s", info.Name())
			return nil
		}
		glog.V(3).Infof("Discovering device in %s", path)
		var a DeviceStruct
		...
		a.did = did
		a.slot = info.Name()
		deviceClass := formatDeviceID(vid, did)
		devicesSet[deviceClass] = a
...
func getIOMMUGroup(deviceAddress string) (int, error) {
	s := strings.Split(deviceAddress, "_")
	iommuPath, err := os.Readlink(filepath.Join("/sys/bus/pci/devices", s[2], "iommu_group"))
	if err != nil {
