...
func (dpi *VFIODevicePlugin) ListAndWatch(e *pluginapi.Empty, s pluginapi.DevicePlugin_ListAndWatchServer) error {
	...
	filepath.Walk("/sys/bus/pci/devices", func(path string, info os.FileInfo, err error) error {
		...
		if vendorID == dpi.vendorID {
			glog.V(3).Infof("Discovering vendorID in %s", vendorID)
			glog.V(3).Infof("Discovering deviceID in %s", deviceID)
			glog.V(3).Infof("Discovering dpi.vendorID in %s", dpi.vendorID)
			glog.V(3).Infof("Adding device in %s", path)
			deviceClass := strings.Join([]string{vendorID, deviceID, info.Name()}, "_")
			//deviceClass := formatDeviceID(vendorID, deviceID)
			glog.V(3).Infof("deviceClass %s", deviceClass)
